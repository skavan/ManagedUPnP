...
namespace ManagedUPnPTest
{
    ...
    public partial class ctlUPnPTreeBrowser : TreeView
    {
        ...
        /// <summary>
        /// Adds a device to the tree view at the correct location.
        /// </summary>
        /// <param name="device">The service to add.</param>
        public void AddDevice(Device ldDevice)
        {
            // Get the device and device node for the service
            //Device ldDevice = service.Device;
            TreeNodeCollection lnDeviceNode = GetDeviceNodeFor(ldDevice);

            // Add the service node
            foreach (Service service in ldDevice.Services)
            {
                TreeNode lnServiceNode = AddUPnPItem(lnDeviceNode, new UPnPServiceTreeItem(service));
                // Get the service description
                ServiceDescription lsdDesc = service.Description();
            ...
            foreach (Device device in ldDevice.Children)
            {
                TreeNodeCollection chDeviceNode = GetDeviceNodeFor(device);

                        // Add the service node
                foreach (Service service in device.Services)
                {
                    TreeNode lnServiceNode = AddUPnPItem(chDeviceNode, new UPnPServiceTreeItem(service));
                    // Get the service description
                    ServiceDescription lsdDesc = service.Description();
                ...
                //TreeNode lnDeviceNode = AddUPnPItem(lnDeviceNode, new UPnPServiceTreeItem(service));
                //device.Description
            }
        }
